...
// 
// StatsMonitorWidgetEditor.cs - version 1.0.0 - Created 11/13/17 23:30
//  
...
{
	[CustomEditor(typeof(StatsMonitorWidget))]
	internal sealed class StatsMonitorWidgetEditor : UnityEditor.Editor
	{
		// ----------------------------------------------------------------------------------------
		// Properties
		// ----------------------------------------------------------------------------------------

		private StatsMonitorWidget _self;
		...
		private SerializedProperty _gcMonitoringEnabled;
		...
		{
			_self = (target as StatsMonitorWidget);
			...
			_gcMonitoringEnabled  = serializedObject.FindProperty("gcMonitoringEnabled");
		...
		public override void OnInspectorGUI()
		{
			...
			/* General parameters section */
			if (EditorUtil.PropertyChanged(_status, new GUIContent("Status",
				...
				_self.Status = (StatsMonitorStatus) _status.enumValueIndex;
			...
			if (EditorUtil.PropertyChanged(_renderMode, new GUIContent("Render Mode",
				...
				_self.RenderMode = (UIRenderMode) _renderMode.enumValueIndex;
			
			if (EditorUtil.PropertyChanged(_layout, new GUIContent("Layout",
				...
				_self.Layout = (StatsMonitorLayout) _layout.enumValueIndex;
			
			if (EditorUtil.PropertyChanged(_style, new GUIContent("Style",
				"Provides a selection of preset styles that affect the background color transparency and text/graph effects.")))
			{
				_self.Style = (StatsMonitorStyle) _style.enumValueIndex;
				_colorBGUpper.colorValue = _self.ColorBgUpper;
				_colorBGLower.colorValue = _self.ColorBgLower;
				_colorGraphBG.colorValue = _self.ColorGraphBG;
				_colorOutline.colorValue = _self.ColorOutline;
			}
			
			if (EditorUtil.PropertyChanged(_alignment, new GUIContent("Alignment",
				"Determines the position of the stats monitor.")))
				_self.Alignment = (UIAlignment) _alignment.enumValueIndex;
			
			if (EditorUtil.PropertyChanged(_keepAlive,
				new GUIContent("Keep Alive", "If checked prevents the stats monitor object from being destroyed on level (scene) load.")))
				_self.KeepAlive = _keepAlive.boolValue;
			
			if (EditorUtil.PropertyChanged(_statsUpdateInterval,
				new GUIContent("Stats Update Interval", "The time, in seconds at which the text displays are updated.")))
				_self.StatsUpdateInterval = _statsUpdateInterval.floatValue;
			
			if (EditorUtil.PropertyChanged(_graphUpdateInterval,
				new GUIContent("Graph Update Interval", "The time, in seconds at which the graph is updated.")))
				_self.GraphUpdateInterval = _graphUpdateInterval.floatValue;
			
			if (EditorUtil.PropertyChanged(_objectsCountInterval,
				new GUIContent("Object Count Interval", "The time, in seconds at which objects are counted. Set to 0 to disable object counting.")))
				_self.ObjectsCountInterval = _objectsCountInterval.intValue;

			EditorGUILayout.Space();

			if (EditorUtil.PropertyChanged(_gcMonitoringEnabled, new GUIContent("GC Monitoring", "If enabled, the graph will monitor the garbage collection count and draws a blip everytime a GC occurs.")))
				_self.gcMonitoringEnabled = _gcMonitoringEnabled.boolValue;
			...
			if (EditorUtil.Foldout(_hotKeyGroupToggle, "Hot Keys"))
			{
				...
				EditorGUILayout.PropertyField(_modKeyToggle,
					new GUIContent("Toggle Modifier Key",
						"Optional modifier key used in combination with the toggle hot key. Set this to None if you want to use the hot key without modifier key."));
				EditorGUILayout.PropertyField(_hotKeyToggle,
					new GUIContent("Toggle Hot Key", "The hot key used for toggling the stats monitor visibility."));
				EditorGUILayout.PropertyField(_modKeyAlignment,
					new GUIContent("Alignment Modifier Key",
						"Optional modifier key used in combination with the alignment hot key. Set this to None if you want to use the hot key without modifier key."));
				EditorGUILayout.PropertyField(_hotKeyAlignment,
					new GUIContent("Alignment Hot Key",
						"The hot key used for switching between the alignment modes (see Alignment parameter)."));
				EditorGUILayout.PropertyField(_modKeyStyle,
					new GUIContent("Style Modifier Key",
						"Optional modifier key used in combination with the style hot key. Set this to None if you want to use the hot key without modifier key."));
				EditorGUILayout.PropertyField(_hotKeyStyle,
					new GUIContent("Style Hot Key", "The hot key used for switching between the different styles (see Style parameter)."));
			}
			...
			{
				EditorGUI.indentLevel = 1;
				if (EditorUtil.PropertyChanged(_fontFace,
					new GUIContent("Font", "The font used for all text fields. Leave blank to use the included font.")))
				...
				if (EditorUtil.PropertyChanged(_autoScale,
					new GUIContent("Autoscale",
						"If turned on, scale will be determined according to current screen DPI. Recommended to leave on for testing on various supported devices that have large differences in screen resolution."))
				)
					_self.AutoScale = _autoScale.boolValue;
				GUI.enabled = !_autoScale.boolValue;
				if (EditorUtil.PropertyChanged(_scale,
					new GUIContent("Scale", "Allows to scale up the size of the stats monitor view by a multiplier.")))
					_self.Scale = _scale.intValue;
				GUI.enabled = true;
				if (EditorUtil.PropertyChanged(_colorBGUpper,
					new GUIContent("BG Color Top", "The color and transparency of the background panel top.")))
					_self.ColorBgUpper = _colorBGUpper.colorValue;
				if (EditorUtil.PropertyChanged(_colorBGLower,
					new GUIContent("BG Color Bottom", "The color and transparency of the background panel bottom.")))
					_self.ColorBgLower = _colorBGLower.colorValue;
				if (EditorUtil.PropertyChanged(_colorGraphBG,
					new GUIContent("Graph Background Color", "The background color and transparency of the graph area.")))
				...
				if (EditorUtil.PropertyChanged(_colorFPSWarning,
					new GUIContent("FPS Warning Color", "The FPS counter will use this color if FPS falls below the FPS warning threshold.")))
					_self.ColorFPSWarning = _colorFPSWarning.colorValue;
				if (EditorUtil.PropertyChanged(_colorFPSCritical,
					new GUIContent("FPS Critical Color", "The FPS counter will use this color if FPS falls below the FPS critical threshold.")))
				...
				if (EditorUtil.PropertyChanged(_colorGCBlip,
					new GUIContent("GC Blip Color", "Color used for garbage collection graph blip.")))
				...
				if (EditorUtil.PropertyChanged(_colorObjCount,
					new GUIContent("Objects Count Color",
						"Color used for the 'currently rendered objects' / 'total render objects' / 'total game objects' values.")))
					_self.ColorObjectCount = _colorObjCount.colorValue;
				if (EditorUtil.PropertyChanged(_colorMemTotal,
					new GUIContent("Total Memory Color", "Color used for the total memory value.")))
					_self.ColorMemTotal = _colorMemTotal.colorValue;
				if (EditorUtil.PropertyChanged(_colorMemAlloc,
					new GUIContent("Allocated Memory Color", "Color used for the allocated memory value.")))
				...
				if (EditorUtil.PropertyChanged(_colorSysInfoEven,
					new GUIContent("System Info Even Color", "Color used for even sysinfo rows.")))
					_self.ColorSysInfoEven = _colorSysInfoEven.colorValue;
				if (EditorUtil.PropertyChanged(_colorOutline,
					new GUIContent("Outline Color",
						"Color used for text and graph outline. Setting alpha to 0 will remove the effect components.")))
			...
			if (EditorUtil.Foldout(_fpsGroupToggle, "FPS Specific"))
			{
				...
				if (EditorUtil.PropertyChanged(_throttleFrameRate,
					new GUIContent("Throttle FPS",
						"Enables or disables FPS throttling. This allows to see how the game performs under a specific frame rate. Does not guarantee selected frame rate. Set this to -1 to render as fast as possible in current conditions.\n\nIMPORTANT: this option disables VSync while enabled!"))
				)
				...
				if (EditorUtil.PropertyChanged(_avgSamples,
					new GUIContent("Average FPS Samples",
						"The amount of samples collected to calculate the average FPS value from. Setting this to 0 will result in an average FPS value calculated from all samples since startup or level load."))
				)
					_self.AverageSamples = _avgSamples.intValue;
				if (EditorUtil.PropertyChanged(_warningThreshold,
					new GUIContent("FPS Warning Threshold", "The threshold below which the FPS will be marked with warning color.")))
					_self.WarningThreshold = _warningThreshold.intValue;
				if (EditorUtil.PropertyChanged(_criticalThreshold,
					new GUIContent("FPS Critical Threshold", "The threshold below which the FPS will be marked with critical color.")))
	...
	{
		public static bool PropertyChanged(SerializedProperty property, GUIContent content = null, params GUILayoutOption[] options)
		{
		...
		{
			var fRect = EditorGUILayout.BeginHorizontal();
			fRect.xMin += 11;
